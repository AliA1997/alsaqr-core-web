// Images with fallbacks. Ported from the reference repo's src/common/Image.tsx.
import { MouseEventHandler, useState } from "react";
import {
  FALLBACK_IMAGE_URL,
  FALLBACK_NEWS_IMAGE_URL,
...
export function FallbackImage({ src, alt, onClick }: CommonImageProps) {
  return (
    <img
...
export function OptimizedImage({
  ...
  return (
    <img
      ...
      onClick={onClick}
      onError={() => {
        if (imageUrl != FALLBACK_IMAGE_URL) setImageUrl(FALLBACK_IMAGE_URL);
      }}
      loading="lazy"
    />
  );
