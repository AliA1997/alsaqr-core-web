// Accordion. Ported from the reference repo's src/common/Accordion.tsx.
    ...
    <div className={`w-full divide-y ${className}`}>
      {items.map((item, index) => (
        <div key={index} className="group">
          ...
          >
            <h4 className="font-medium text-gray-700 dark:text-gray-50">
              {item.title}
            </h4>
            <motion.div
          ...
          >
            <div id={`accordion-section-${index}`} className="px-4 pb-4">
              {item.jsx}
            </div>
          </motion.div>
        </div>
