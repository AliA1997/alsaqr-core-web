...
export const ContentContainerWithRef = ({
  ...
  return (
    <div
      ref={innerRef}
      className={`max-w-4xl mx-auto bg-white dark:bg-[#0e1517] rounded-lg mt-10 ${
...
export function MessagesImagePreview({ user, index }: MessagesImagePreviewProps) {
  return (
    <OptimizedImage
      ...
      src={user.avatar}
      alt={user.username}
      classNames={`w-10 h-10 rounded-full border-2 border-white dark:border-gray-800 relative ${
...
export function InfoCardContainer({
  ...
  return (
    <div
      className={`relative flex flex-1 flex-col border-y border-gray-100 p-5 hover:shadow-lg dark:border-gray-800 dark:hover:bg-[#000000] dark:text-gray-50 ${
        classNames && classNames
      }`}
    >
      {children}
